...
- **这里不挂状态灯**（原来挂过，后来撤了，见坑 69）—— 状态只由上面那条状态行说。
  它只回答"**这是谁说的**"。
...
68. **★ vitest 会偶发"少报已跑用例数"，别只看那一行的总数**（2026-09-16 踩到） → 同一个工作区、同一批文件，
   某一次全量跑报 `Tests 299 passed`，之后两次连跑稳定 **303**，而那两个"变多"的文件（`resume.spec` 18→21、
   `runs-transport.spec` 16→17）**内容一个字没改**、`it(` 条数本来就是 21/17。
   教训：① 看到"用例数变了"先别当成回归，`grep -c "^\s*it(" <file>` 与报告对账、再连跑两次看是否稳定；
   ② 报数只能当参考，**用它判断"有没有跑全"不如用它判断"有没有红"**。
69. **★★ 同一件事不要在"轮首"和"状态行"都说**（2026-09-16 实测后的结论，撤掉了轮首灯） → v2.14 一度在每轮开头
