...
7. **审批卡片**（`/v1/runs` 通道能力，**2026-09-15 已真机验收**）：让 agent 做需要人工批准的操作时，状态条变成 `等待你批准：<工具名>`，下面出现蓝色卡片（工具名 + 被标红的命令原文 + `批准一次` / `本会话都允许` / `永久允许` / `拒绝`），这一轮**不会自己往下走**。点 `批准一次` 后服务端继续执行、卡片显示"已回话"。**注意不是每条危险命令都会弹卡片**，三个条件要同时满足：①命中危险模式；②该模式的描述**不在** `config.yaml` 的 `command_allowlist` 里；③没被 smart approval（辅助模型）自动放行。已验收的无害触发命令：`chmod -R 777 <一次性目录>`（模式描述 `world/other-writable permissions`）。"静默执行了"多半是第 ② 条：本机 allowlist 里有 `delete in root path`，所以 `rm -rf /绝对路径` 永远不会弹卡片。想让所有通道整体退回旧行为：把 `src/stores/chat.ts` 的 `DEFAULT_TRANSPORT` 改成 `'stream'`
...
| 危险操作**只显示"被安全闸门拦下"而不是审批卡片** | 三种可能：①该危险模式的描述已在本机 `config.yaml` 的 `command_allowlist` 里（`delete in root path` / `execute_code` / `script execution via heredoc` / `script execution via -e -c flag` / `git reset --hard`）→ 服务端在"问人"**之前**就静默放行，本来就没有卡片可弹；②被 smart approval（辅助模型）自动放行；③仍在走旧通道（`DEFAULT_TRANSPORT='stream'`，那条路没接审批线，会被当场 fail-closed 拒） |
