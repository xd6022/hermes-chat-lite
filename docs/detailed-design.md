...
5. **状态行只有一行**（2026-09-17 用户要求"灯和下面的模型信息合成一行"）：
   `灯 + 状态词` 与 `模型 │ 窗口 1m │ 本轮输入合计` **同属一个 flex 容器** —— `ContextGauge.vue` 现在作为
   **子组件**渲染在状态行里（不再自己占一行），**自带前导 `│`**；没有任何数据时整段不渲染，**不留孤零零的分隔符**。
   两段行高统一 `leading-4`（16px）⇒ 盒顶对齐（真浏览器实测 `top` 差 **0px**，探针按 ≤1px 判）。
...
1. 连接状态：绿点 + 服务端版本（实际值来自 `GET /health`，如 `Hermes v0.21.3`）+ 刷新按钮；
   **前端构建标识**（`前端构建 {短 sha} · {日期}`）也在这里 —— 2026-09-17 起从顶栏搬过来（首屏不摆构建信息）。
2. **外观**：当前主题（白天/黑夜）+ 切换按钮（等同顶栏那个按钮）。
3. **头像**：默认 `-_-`，可改（存浏览器，改完立刻生效、不用重新构建；见 `lib/appearance.ts`）。
4. 关于：版本号、一组快捷键说明。
...
46. **★★ 手机上"刷了好几次还是老版本" = 入口 HTML 被缓存**（v2.2.3）→ 症状：服务端镜像早换了新产物（旧 `index-*.js` 在容器里已经 404），手机上刷新、杀进程重开都还是旧界面。根因：`location /` 里**没给 `index.html` 任何 `Cache-Control`**（只有 `ETag`/`Last-Modified`），浏览器于是按 `Last-Modified` 做"启发式缓存"，自带 webview 缓存/云加速的国产浏览器更激进；而 `/assets/`（带 hash）那边是 `max-age=604800, immutable`（**这是对的，别动**）→ "旧 HTML + 旧 hash 资源"这套组合被一直用下去，链子断在第一环。修法：`location = /index.html` 里 `Cache-Control: no-store, no-cache, must-revalidate` + `Pragma: no-cache` + `etag off` + `if_modified_since off`（这份 HTML 才 1.5KB，不值得为它省一次请求）；带 hash 的资源保持长缓存 —— 只要入口 HTML 每次是新的，新 HTML 自然会带出新的 hash 文件名。真机上想**立刻绕过**：地址后面加个查询串（`?v=2` —— 缓存按完整 URL 存，必然重新拉 HTML），或开无痕标签页/清该站点缓存。改完若**仍然**被缓存，那说明是浏览器自带的"云加速/极速模式"在缓存（关掉它或换浏览器）；判据是先 `curl -I` 确认入口 HTML 真的带上了 `no-store`。
    **配套**：界面顶部会显示**前端构建标识**（`vite.config.ts` 构建时自动生成：本机构建是"短 sha · 北京时间"，Docker 镜像里没有 `.git` 所以只有时刻）——
   **2026-09-17 起只在 Settings 的「连接状态」里显示**（顶栏与空态那两份都已撤掉：首屏不摆构建信息）；
   部署完打开手机 → Settings 对一眼就知道有没有吃到新版，不用再靠猜。
...
71. **★ 探针里 `page.evaluate(fn)` 的函数必须"自包含"**（2026-09-16、2026-09-17 各踩一次） → 传进去的函数会被
   **序列化后送进页面执行**，它闭包里的外部函数/常量在页面里根本不存在 ⇒ `ReferenceError: lastTurnText is not defined`。
   写法：要在页面里跑的逻辑**整段写进 `page.evaluate(() => {...})` 内部**；需要外部值就显式当第二个参数传。
72. **★ "同一行"这类断言别只写"像素完全相等"，更别用放宽断言掩盖差异 —— 先把那 2px 查清楚**（2026-09-17） →
   探针报「水位与灯 `top` 不等（774 vs 776）」时，第一反应容易是"探针太严"；一查是**真差异**：水位段当时是
   `text-[11px]/leading-5`（20px 行高）、状态词是 `text-xs`（16px）⇒ 盒高不同，`items-center` 下顶边错开 2px。
...
77. **★ 两个「看着无关」的坑：查询串里的 `+`、以及备份名撞车**（2026-09-22，消息中心 v3.1） →
   ① 时间参数 `since=2026-09-22T12:00:00+08:00` **手工拼进 URL 会被解析成空格**（`+` 在查询串里表示空格）
   ⇒ 服务端 400。**必须走 `URLSearchParams`**（会编成 `%2B`）。可复用：凡是值里可能带 `+` / 空格 / 中文的查询参数，
   一律交给 `URLSearchParams` 或 curl 的 `--data-urlencode`，别自己拼字符串。
   ② 写 RED 核验脚本时，备份名用 `basename` 会让**同名但不同目录**的文件互相覆盖
   （`src/api/inbox.ts` 与 `src/stores/inbox.ts`）⇒ 还原时把 A 的内容写进 B，build 直接挂。
   备份路径要**带子目录**：`cp "$f" "$BK/$f.bak"`。
