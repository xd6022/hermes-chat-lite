...
| P24 静态资源"缺文件"真 404 加固（v2.10，同 `feat/phase-dispatch-steer`，随 PR #18 部署） | ✅ 完成并部署（2026-09-15 线上复验） | `nginx.conf` 加根层后缀 location（`try_files $uri =404`）+ `Dockerfile` 加构建期断言（`test -f dist/{index.html,favicon.svg,favicon.ico,apple-touch-icon.png}` + `test -d dist/assets`） | 后缀正则语义 18/18 + 断言 **RED/GREEN**（模拟漏拷 `public/`、产物无 `assets/` 均按预期失败）+ **真 nginx 解析器 crossplane `status: ok` / 0 错**；**线上复验（2026-09-15，容器内网绕开 Caddy）**：`/nope.svg` **200 → 404**、`/favicon.svg` 200、`/assets/nope.js` 404 ⇒ 行为层通过；自检命令见 §8.4 |
| P25 按时间交错渲染（v2.11，`feat/interleaved-render`，PR #19） | ✅ 完成并部署（2026-09-15） | `UiMessage.kind`（`text`/`tools`）+ `normalize()` 改为按时间切段 + `prependEarlier()` 只在同 kind 合并 + `TurnCtx.segs` 与 `openTextSeg/sealTextSeg/toolsSegFor/replaceTurnSegs`（流式期即交错、轮末用 transcript 整批重建）+ 新 `lib/turns.ts`（按轮分组）+ `MessageItem` 删折叠开关、工具行改常驻小字行 + `ChatWindow` 按轮渲染（轮内 `space-y-2`／轮间 `mt-6`） | 单测 **245**（新增 `lib/turns.spec.ts` 5 条、改写 12 条旧断言）+ **RED/GREEN（还原 3 个源文件 → 12 条核心断言全红）** + `vite build`（`工具调用 (` 0 命中）+ **真浏览器**：段序与服务端 transcript **逐轮完全一致**（11/11）、工具行 0 折叠开关且全部可见、轮内 8px < 轮间 24px、实时一轮中途即 `text→tools` 跑完 `text→tools→text`、滚动锚定无回归（落底 距底 0、上滚不被拽回）；**线上复验（部署后同一套脚本打 `http://chatlite`）**：产物 `index-B3kT_XwN.js`（`工具调用 (` **0 命中**）、段序 **11/11 轮一致**、0 折叠开关、工具行 14/14 可见、轮内 8px < 轮间 24px、线上实时 `[text,tools] → [text,tools,text]`、落底 距底 0、`pageerror` 无。详见 §5.13 |
