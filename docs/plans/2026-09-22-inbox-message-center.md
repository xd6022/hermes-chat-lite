...
| 动作 | `标记已读` / `归档` / **`就这条问 agent`**（把正文当上下文起一轮 `/v1/runs`，走现有发送链路）/ **`一键已读 (N)` 放在【抽屉头部】** / `立即刷新` |
| 筛选 | 全部 / 股票信号 / 邮件 / 提醒 / 系统（按 `category`，5 项） |
| **起始时间窗（v3.1）** | `只看 [datetime-local]` + 「今天 0 点」「不限」；默认 **当天 00:00**；**同时作用于列表与徽标**（不同口径就会出现"徽标 11 但列表 3 条"那种困惑）；值**只在内存里**（刷新即还原默认，用户明确"不用保存"）；空态分两种说法 |
| **展开即已读（v3.1）** | 点开看正文就自动标已读（用户要求）；**例外**：本会话里被手动「标为未读」的不会被吃掉（显式动作优先） |
...
### §12.9 「一键已读」修复（2026-09-22 用户反馈后）

**用户原话**："得增加一个一键已读的功能，我当前看了消息，但是外面还是显示 11 条未读"。

**先说查证结果**：功能**本来就有**（`inbox-read-all`），但放在**抽屉最底部右下角、一行很小的灰字** ⇒ 用户根本找不到
（线上 bundle 里 `全部已读` 确实命中 1）。**找不到就等于没有** —— 这是一次纯发现性问题，不是缺失功能。

**顺手查出一个真 bug**：`readAll()` 当时把**当前筛选**当 category 传给服务端 ⇒ 筛着「邮件」时点它只会清邮件类，
徽标停在非 0，用户看到的现象就是"点了没用"。已改成**不带筛选**（按钮标签就是"全部"；接口仍保留 category 参数，界面不再暴露）。

**改法**：
1. 按钮挪到**抽屉头部**（`消息 · 11 条未读 · [一键已读 (11)] …`），做成有边框的小按钮、未读为 0 时禁用且不带数字；
2. 页脚只留「上次更新」（**不做重复功能** —— 同一个按钮两处放是用户明确反感的事）；
3. 手机宽度下轮询提示 `hidden sm:inline`（头部别挤）。

**验证**：单测 369（新增 4 条）+ **RED 核验**（改坏 3 处：带筛选 / 按钮不带数字 / 未读 0 仍可点 ⇒ **4 条断言变红**）+
**真浏览器 11/11**（探针 `/opt/data/.verify/probe_readall_smoke.cjs`：打**冒烟表**、不碰正式消息 ——
徽标=库内未读数、按钮在头部且带 `(3)`、点击后**库里三行全落 read_at**、外部徽标消失、头部改「全部已读」、请求不带 category、无 pageerror）。

⚠️ 测试桩的坑（这次踩到）：`url.startsWith('/inbox/messages/')` 会把 `/inbox/messages/read-all` 也吞掉，
兜底返回了详情形状（没有 `unread_count`）⇒ store 里 `unread` 变 `undefined`、断言假红。**写桩时把固定路径放前面**。

### §12.10 起始时间窗 + 展开即已读（v3.1，2026-09-22 用户提出）

用户口径：默认**当天 00:00**；前端把这个时间传给后端，只查它之后的消息；可以手改成别的时间；
**刷新页面就还原成当天 0 点，不用保存**。另外要求"展开即已读"。

**实现**：
- 服务端 `GET /inbox/messages` 与 `GET /inbox/unread-count` 新增 `since`（带偏移的 ISO，统一换算成
  Asia/Shanghai naive 再与 `occurred_at` 比；空=不限；**解析不了给 400**，不静默当不限）。
  两个接口都带 ⇒ **徽标数字与列表同口径**（否则会出现用户已经踩过一次的"外面 11、里面 3 条"）。
- 前端 `lib/inboxSince.ts`（纯函数：默认值/解析/API 串/人话标签）+ `stores/inbox.ts` 的 `sinceInput`
  （**内存态**，不写 localStorage —— 写盘就把"刷新还原"变成 bug）+ 抽屉里一行 `datetime-local` + 两个快捷位。
- 「展开即已读」放在 `toggleDetail` 成功之后；被手动标为未读的进 `unreadExempt`，不再被自动吃掉。

**验证**：
| 层 | 结果 |
| --- | --- |
| 服务端（冒烟表，15 项） | ✅ 不传/空=不限、带偏移与不带偏移都认、12:00 之后只剩 14:35 那条、未读接口同口径、非法值 400、与 category 组合 |
| 单测 **392**（+23） | ✅ 新增 `lib/inboxSince.spec`（默认值/UTC 陷阱/偏移/文案）+ store（带 since、改时间即重拉、不限不带参、刷新还原且不写盘）+ drawer（默认值、改时间、不限、今天 0 点、空态两种说法） |
| RED 核验（6 处改坏） | ✅ **9 条断言变红**（请求不带 since / 丢偏移 / 默认值用 UTC / 去掉自动已读 / 去掉豁免 / 空态不分情况） |
| 真浏览器（冒烟表，**20 项**） | ✅ 默认当天 0 点 3 条 → 改 12:00 剩 1 条且徽标同步 →「不限」3 条且请求不带 since →「今天 0 点」复位 → 18:00 空态说清原因 → **刷新后窗口还原** → 展开即已读（库里落 read、徽标 3→2、按钮变「标为未读」）→ 无 pageerror |

⚠️ **踩到的两个坑（都写进 detailed-design 坑 77）**：
1. `since=2026-09-22T12:00:00+08:00` 里的 `+` **手工拼进查询串会被解析成空格** ⇒ 服务端 400。
