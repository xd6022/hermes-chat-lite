...
```bash
node node_modules/vitest/vitest.mjs run          # 69 个用例：SSE 半帧切片、中文多字节切分、过滤规则、输入法、集成、搜索/折叠/主题、历史分页与消息合并
...
5. 打开一个长会话（上百条）→ 底部应该出现 `加载更早的消息` 按钮，点几下能翻到会话**第一条**；同一轮回答的多段文字应当是**连成一段**的（不再是一段一段分开、中间空行）；顶部若出现 `内部上下文摘要（Hermes 压缩边界）` 说明该会话被上下文压缩过，默认折叠
